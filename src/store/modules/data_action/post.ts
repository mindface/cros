...
export function postReducer(state:any = initalLevelState(), action:LevelPostAction) {
  switch (action.type) {
...
export const postLevelData = (sendData:Levels) => {
  ...
  const res = axios.post('http://localhost:3003/api/levels/', postSendData)
    .then( (res) => {
    const getdata:any = res.data
...
export const getLevelData = () => {
  ...
  return (dispatch: Dispatch)  => {
    ...
    // axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';
    axios.get('http://localhost:3003/api/levels/',headers)
      .then( (res) => {
